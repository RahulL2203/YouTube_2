...
const Head = () => {
  ...
  return (
    ...
    <div className='flex col-span-1 pb-2 ml-2'>
      <img 
      ...
      alt="YouTube_logo" 
      src= "https://w1.pngwing.com/pngs/66/171/png-transparent-google-logo-youtube-youtuber-youtube-rewind-text-area-line.png"/>

    </div>
    <div className='col-span-10 px-10'>
      <div>
      <input className ="w-1/2 px-5 border border-gray-400 p-1 rounded-l-full"
       type='text'
       value={searchQuery}
       onChange={(e)=>setSearchQuery(e.target.value)}
       onFocus={()=>setShowSuggestions(true)}
       onBlur={()=>setShowSuggestions(false)}/>
      <button className ="border border-gray-400 p-1 px-3 rounded-r-full bg-gray-100">🔍
      </button>
      </div>
       {showSuggestions && (<div className= "fixed bg-white py-2 px-5 w-[28rem] ml-4 border border-gray-100 shadow-lg rounded-lg">
       <ul>
       {suggestions.map((sug) => (
        <li key ={sug} className="px-3 py-1 hover:bg-gray-200 shadow-sm rounded-lg">
          🔍 {sug}
          </li>
          ))}
